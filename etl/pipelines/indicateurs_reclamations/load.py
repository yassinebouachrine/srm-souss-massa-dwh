...
- Les lignes Recap DP utilisent id_centre = 0 (centre sentinelle 'AGREG_DP')
"""
...
SOURCE_SAISIE = CONFIG["sources"][SOURCE_NAME]["source_saisie"]
...
def _resolve_sk_indicateurs(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Résout les SK pour les indicateurs (agrégation DP → id_centre=0)."""
    ...
    # id_centre : soit le centre réel, soit 0 (agrégation DP) si est_recap_dp
...
def _resolve_sk_reclamations(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Résout les SK pour les réclamations."""
    ...
    df = df.copy()
    df["id_dp"]   = df["code_dp"].map(map_dp)
    df["id_type"] = df["code_reclamation"].map(map_reclam)
...
def load_indicateurs(df: pd.DataFrame) -> dict:
    """Charge les indicateurs dans le DWH (UPSERT)."""
    if df.empty:
    ...
    records = [
        {
            ...
            "onglet_source":      _to_pg_str(row["onglet_source"]),
            "date_chargement":    now,
        }
...
def load_reclamations(df: pd.DataFrame) -> dict:
    """Charge les réclamations dans le DWH (UPSERT)."""
    if df.empty:
    ...
    records = [
        {
            ...
            "nb_reclamations":         _to_pg_int(row["nb_reclamations"]),
            "temps_moyen_coupure":     _to_pg_float(row["temps_moyen_coupure"]),
            "delai_moyen_traitement":  _to_pg_float(row["delai_moyen_traitement"]),
...
def run_load() -> dict:
    """Exécute le chargement Gold."""
    logger.info("=" * 70)
    logger.info("🥇 GOLD - CHARGEMENT DWH INDICATEURS & RÉCLAMATIONS")
