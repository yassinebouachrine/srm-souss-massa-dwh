...
def get_centres(id_province=None):
    if id_province:
        return execute_query(
            "SELECT id_centre, code_centre, nom_centre, type_centre FROM gold.dim_centre WHERE id_province = %s ORDER BY nom_centre",
            (id_province,),
        )
    return execute_query("SELECT id_centre, code_centre, nom_centre, type_centre FROM gold.dim_centre ORDER BY nom_centre")


def get_types_indicateur():
    return execute_query(
        "SELECT id_type_indicateur, code_indicateur, libelle_indicateur, unite, categorie FROM gold.dim_type_indicateur_dp ORDER BY categorie, code_indicateur"
    )


def get_types_reclamation():
    return execute_query(
        "SELECT id_type_reclamation, code_type, libelle_reclamation, categorie_reclamation, est_comptage, est_duree FROM gold.dim_type_reclamation ORDER BY categorie_reclamation, code_type"
    )
...
def update_staging_status(table, lot_id, new_status, user_id, comment=None, level="dp"):
    tbl = "app_staging.staging_indicateurs_dp" if table == "indicateurs" else "app_staging.staging_reclamations_dp"
    if level == "dp":
        execute_insert(
            f"""
            UPDATE {tbl}
...
# ════════════════════════════════════════════════════════════════
# CORRECTION DE DONNÉES REJETÉES
# ════════════════════════════════════════════════════════════════

def get_rejected_lots(id_province=None, data_type="indicateurs"):
    """Récupère les lots rejetés (par DP ou régional) pour correction."""
    table = "app_staging.staging_indicateurs_dp" if data_type == "indicateurs" else "app_staging.staging_reclamations_dp"
...
def update_staging_record(table, id_staging, field_updates: dict):
    """Met à jour un enregistrement individuel dans le staging."""
    tbl = "app_staging.staging_indicateurs_dp" if table == "indicateurs" else "app_staging.staging_reclamations_dp"
    set_clauses = []
    params = []
...
def reset_lot_for_correction(table, lot_id):
    """Remet un lot rejeté en statut 'brouillon' pour correction."""
    tbl = "app_staging.staging_indicateurs_dp" if table == "indicateurs" else "app_staging.staging_reclamations_dp"
    execute_insert(
        f"""
        UPDATE {tbl}
        SET statut = 'brouillon',
            valide_par_dp = NULL, date_validation_dp = NULL, commentaire_dp = NULL,
            valide_par_regional = NULL, date_validation_reg = NULL, commentaire_regional = NULL,
            date_modification = CURRENT_TIMESTAMP
...
def transfer_validated_indicateurs_to_gold(lot_id):
    execute_insert(
        """
        ...
        SELECT
            dt.id_temps, s.id_province, s.id_centre, ti.id_type_indicateur,
            s.valeur_mensuelle, s.valeur_recapitulatif, 'STREAMLIT_SAISIE', s.lot_id, CURRENT_TIMESTAMP
        FROM app_staging.staging_indicateurs_dp s
...
def transfer_validated_reclamations_to_gold(lot_id):
    execute_insert(
        """
        ...
        SELECT
            dt.id_temps, s.id_province, s.id_centre, tr.id_type_reclamation,
            s.nombre_reclamations, s.temps_moyen_coupure_h, s.delai_moyen_traitement_j,
            s.valeur_brute, CURRENT_TIMESTAMP
        FROM app_staging.staging_reclamations_dp s
        JOIN gold.dim_temps dt ON dt.annee = s.annee AND dt.mois = s.mois
...
def get_dashboard_stats(id_province=None):
    ...
    indic_stats = execute_query(
        f"SELECT statut, COUNT(*) as count FROM app_staging.staging_indicateurs_dp {prov_filter} GROUP BY statut",
        params,
    )
    reclam_stats = execute_query(
        f"SELECT statut, COUNT(*) as count FROM app_staging.staging_reclamations_dp {prov_filter} GROUP BY statut",
        params,
    )
    return {
...
def get_audit_logs(user_id=None, limit=50):
    if user_id:
        return execute_query(
            "SELECT al.*, u.nom_complet, u.username FROM app_auth.audit_logs al JOIN app_auth.utilisateurs u ON al.id_utilisateur = u.id_utilisateur WHERE al.id_utilisateur = %s ORDER BY al.date_action DESC LIMIT %s",
            (user_id, limit),
        )
    return execute_query(
        "SELECT al.*, u.nom_complet, u.username FROM app_auth.audit_logs al JOIN app_auth.utilisateurs u ON al.id_utilisateur = u.id_utilisateur ORDER BY al.date_action DESC LIMIT %s",
        (limit,),
    )


# À AJOUTER À LA FIN DE db/queries.py

def get_draft_lots(id_province=None, data_type="indicateurs"):
    """Récupère uniquement les lots en brouillon."""
    table = "app_staging.staging_indicateurs_dp" if data_type == "indicateurs" else "app_staging.staging_reclamations_dp"
    conditions = ["statut = 'brouillon'"]
    params = []
    if id_province:
        conditions.append("id_province = %s")
        params.append(id_province)
...
def delete_lot(table, lot_id):
    """Supprime définitivement un lot de brouillon."""
    tbl = "app_staging.staging_indicateurs_dp" if table == "indicateurs" else "app_staging.staging_reclamations_dp"
    execute_insert(
